...
router.post('/find-id', (req, res) => {
  ...
  const user = db.prepare('SELECT email FROM users WHERE name = ? AND email = ?').get(name, email);
  if (!user) return res.status(404).json({ error: '일치하는 정보가 없습니다.' });

  // 이메일 일부 마스킹 처리 (예: ab***@naver.com)
...
router.post('/request-code', async (req, res) => {
  ...
  if (!user) return res.status(404).json({ error: '가입되지 않은 이메일입니다.' });
...
router.post('/reset-password', (req, res) => {
  ...
  db.prepare('UPDATE users SET password = ?, updated_at = datetime(\'now\',\'localtime\') WHERE email = ?')
    .run(hash, email);
